...
class TypeAs {
  ...
  constructor(that, options) {
    ...
    this._integer = has(_classes, synonyms.integer) && re[1].test(that);
    this._null = re[2].test(that);
    this._number = has(_classes, synonyms.number) || re[1].test(that);
    this._object = has(_classes, synonyms.object);
    this._promise = has(_classes, synonyms.promise);
    this._real = has(_classes, synonyms.real) && re[0].test(that);
    this._regexp = has(_classes, synonyms.regexp);
    this._string = has(_classes, synonyms.string);
    this._undefined = re[3].test(that);
